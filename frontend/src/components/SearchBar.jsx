import { useEffect, useContext, useRef } from "react";
import { ShopContext } from "../context/ShopContext";
import { assets } from "../assets/frontend_assets/assets";
import { useLocation } from "react-router-dom";

const SearchBar = () => {
  const { search, setSearch, showSearch, setShowSearch } = useContext(ShopContext);
  const location = useLocation();
  const inputRef = useRef(null);

  // Sayfa değişince search barı kapat
  useEffect(() => {
    setShowSearch(false);
  }, [location.pathname]);

  // Search bar açıldığında inputa odaklan
  useEffect(() => {
    if (showSearch && location.pathname.includes("collection")) {
      inputRef.current?.focus();
    }
  }, [showSearch, location.pathname]);

  // Sadece collection sayfasında ve showSearch true ise göster
  if (!(showSearch && location.pathname.includes("collection"))) return null;

  return (
    <div className="border-t bg-gray-50 text-center">
      <div className="inline-flex items-center justify-center border border-gray-400 px-5 py-2 my-5 mx-3 rounded-full w-3/4 sm:w-1/2">
        <input
          ref={inputRef} // buraya ref ekledik
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          type="text"
          placeholder="Search"
          className="flex-1 outline-none bg-inherit text-sm"
        />
        <img src={assets.search_icon} alt="search_icon" className="w-4" />
      </div>
      <img
        onClick={() => setShowSearch(false)}
        src={assets.cross_icon}
        alt="cross_icon"
        className="inline w-3 cursor-pointer"
      />
    </div>
  );
};

export default SearchBar;
